...
* HTTPS는 웹브라우저와 웹서버 사이에 인증을 요구하기 때문에 중간자 공격을 방어할 수 있다.
* 데이터가 탈취되더라도 원본 데이터가 유출될 위험이 줄어든다.
{: .notice--info}

```
(웹브라우저) | -----SSL 세션 연결을 요청----->	| (웹서버)
          |                            	|
          | <--공개키가 담긴 인증서 전달-----	|
		  |                         	|
		  | ---인증서 내 공개키로 암호화한--->	|
...
* 웹브라우저가 SSL 세션 연결을 웹 서버에 요청하면 웹 서버는 유효한 공인된 서버인증서를 웹 브라우저로 전달한다.
* 서버인증서를 전달받은 웹 브라우저는 인증서가 유효한지 확인한 후 이상이 없으면 세션 암호키를 생성한 후 서버로 전달한다.
	* 1회용 암호키 생성
	* 서버인증서에 포함된 서버 공개키로 1회용 암호키를 암호화함
	* 암호화한 1회용 암호키를 웹서버로 전달
	* 서버 개인키로 암호화된 1회용 암호키를 복호화함
* 이 후 부터는 1회용 암호키로 암호화를 하고 데이터를 전송, 수신측에서는 1회용 암호키로 복호화함
{: .notice--info}