...
* UDP에서는 발생하지 않는다 (Sequence 번호 사용)
* Squence number를 스니핑 후 RST 명령을 보내서 Sequence number 초기화
...
* **exploit 코드**
	...
	* 취약점을 이용한 공격의 종류는 BOF, CSRF, XSS  등이 존재한다.
		* CSRF: 용자가 자신의 의지와는 상관없이 공격자가 의도한 수정, 삭제, 등록 행위 등 특정 웹사이트에 요청하게 하는 공격
{: .notice--warning}
